...
The helper uses `ZELLIJ_SOCKET_DIR` when already set. Otherwise it automatically uses a short stable per-user path such as `/tmp/zellij-501`.
...
4. It launches `codex exec` with Sol (`gpt-5.6-sol`) at high reasoning effort in a visible Zellij pane and opens a Ghostty tab attached to it, so you can watch Codex work and interrupt whenever you want.
...
ruby scripts/codex_build.rb --plan docs/plan.md --effort medium
ruby scripts/codex_build.rb --plan docs/plan.md --model another-model
...
In normal use you invoke `/codex-build` and Claude runs the script for you. The defaults are Sol (`gpt-5.6-sol`) and high reasoning effort. `--model` and `--effort` override them, `--session` names the Zellij session, and `--dry-run` prints the assembled brief without launching anything.
...
`--dry-run --feedback "..."` prints the resume command and feedback prompt. Feedback inherits the existing session's model and reasoning effort. Pass `--model` or `--effort` explicitly to change either one; the script checks `codex exec resume --help` before forwarding supported resume flags.
